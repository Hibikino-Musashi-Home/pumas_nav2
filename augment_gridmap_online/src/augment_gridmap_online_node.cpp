...
// Static-furniture obstacle helpers (YAML loader, polygon stamping, markers) by
// ry0hei-k.
#include "augment_gridmap_online/furniture_loader.hpp"
...
public:
  AugmentedGridMapNode() : Node("augment_gridmap_online_node") {
    ...
    // for static furniture obstacle loader
    this->declare_parameter("add_static_obstacles", false);
    ...
    this->get_parameter("add_static_obstacles", add_static_obstacles_);
    this->get_parameter("furniture_pkg", furniture_pkg_);
    ...
    sub_map_ = this->create_subscription<nav_msgs::msg::OccupancyGrid>(
        ...
        std::bind(&AugmentedGridMapNode::callback_save_map, this,
                  std::placeholders::_1));

    sub_point_ = this->create_subscription<geometry_msgs::msg::PointStamped>(
        "point_obstacle", rclcpp::SensorDataQoS(),
        std::bind(&AugmentedGridMapNode::callback_add_point, this,
                  std::placeholders::_1));
    ...
    pub_marker_ = this->create_publisher<visualization_msgs::msg::Marker>(
        ...
        rclcpp::QoS(10).transient_local());
    pub_furniture_marker_ =
        this->create_publisher<visualization_msgs::msg::MarkerArray>(
            make_name("/grid_map/furniture_markers"),
            rclcpp::QoS(10).transient_local());

    // Services
    srv_clear_map_ = this->create_service<std_srvs::srv::Empty>(
        make_name("/clear_map"),
        std::bind(&AugmentedGridMapNode::callback_clear_map, this,
                  std::placeholders::_1, std::placeholders::_2));

    srv_get_augmented_map_ = this->create_service<nav_msgs::srv::GetMap>(
        make_name("/grid_map/get_augmented_map"),
        std::bind(&AugmentedGridMapNode::callback_get_augmented_map, this,
                  std::placeholders::_1, std::placeholders::_2));

    RCLCPP_INFO(
        this->get_logger(),
        "AugmentedGridMap.-> Map enhancer node Initialization finished");
  }

private:
  using FurniturePolygon = augment_gridmap_online::FurniturePolygon;

  // Parameters
  bool use_namespace_, debug_;
  float obstacle_radius_;
  std::string input_map_;
  bool add_static_obstacles_;
  std::string furniture_pkg_;
  std::string furniture_path_;
  std::string furniture_yaml_file_;
  ...
  on_parameter_change(const std::vector<rclcpp::Parameter> &params) {
    ...
    for (const auto &param : params) {
      ...
      else if (param.get_name() == "obstacle_radius")
        obstacle_radius_ = param.as_double();
      else if (param.get_name() == "debug")
        debug_ = param.as_bool();
      else if (param.get_name() == "input_map")
        input_map_ = param.as_string();

      else {
        result.successful = false;
        result.reason =
            "AugmentedGridMap.-> Unsupported parameter: " + param.get_name();
        RCLCPP_WARN(
            this->get_logger(),
            "AugmentedGridMap.-> Attempted to update unsupported parameter: %s",
  ...
  void callback_save_map(const nav_msgs::msg::OccupancyGrid::SharedPtr map) {
    ...
    RCLCPP_INFO(this->get_logger(),
                "AugmentedGridMap.-> Got a map of: [%d,%d] @ %f resolution",
                enhanced_map_.info.width, enhanced_map_.info.height,
                enhanced_map_.info.resolution);

    // Re-apply any previously summoned clicked-point obstacles so they survive
    // a map refresh.
    for (const auto &pt : obstacles_) {
  ...
  void
  callback_add_point(const geometry_msgs::msg::PointStamped::SharedPtr point) {
    if (original_map_.data.empty()) {
      RCLCPP_ERROR(this->get_logger(),
                   "AugmentedGridMap.-> No map data received yet.");
      return;
    }

    if (point->header.frame_id != original_map_.header.frame_id) {
      RCLCPP_ERROR(this->get_logger(),
                   "AugmentedGridMap.-> Frame mismatch: point=%s, map=%s",
    ...
    RCLCPP_INFO(this->get_logger(),
                "AugmentedGridMap.-> Adding point: [%f, %f]", point->point.x,
                point->point.y);
  ...
  void callback_clear_map(const std::shared_ptr<std_srvs::srv::Empty::Request>,
    ...
    RCLCPP_INFO(this->get_logger(),
                "AugmentedGridMap.-> Map restored to original (furniture "
                "obstacles re-applied)");
  ...
  void callback_get_augmented_map(
      const std::shared_ptr<nav_msgs::srv::GetMap::Request>,
      std::shared_ptr<nav_msgs::srv::GetMap::Response> response) {
    RCLCPP_INFO(this->get_logger(),
                "AugmentedGridMap.-> Providing augmented map");
    pub_map_->publish(enhanced_map_);
    pub_metadata_->publish(map_metadata_);
    response->map = enhanced_map_;
  }

  // Stamp a square obstacle footprint into enhanced_map_ at the given point
  // (no obstacle list update).
  void stamp_obstacle_to_map(const geometry_msgs::msg::Point &pt) {
    ...
    int max_x = std::min(static_cast<int>(map_metadata_.width),
                         static_cast<int>(cx + obstacle_radius_ / res));
    int min_y = std::max(0, static_cast<int>(cy - obstacle_radius_ / res));
    int max_y = std::min(static_cast<int>(map_metadata_.height),
                         static_cast<int>(cy + obstacle_radius_ / res));

    if (debug_) {
      RCLCPP_INFO(this->get_logger(),
                  "AugmentedGridMap.-> Cell: [%d, %d,], Obstacle bounds : X "
                  "[%d, %d], Y [%d, %d] ",
  ...
  void add_obstacle_to_map(const geometry_msgs::msg::Point &pt) {
    ...
    if (cx < 0 || cx >= static_cast<int>(map_metadata_.width) || cy < 0 ||
        cy >= static_cast<int>(map_metadata_.height)) {
      RCLCPP_ERROR(this->get_logger(),
                   "AugmentedGridMap.-> Point falls outside map bounds");
      return;
    }
  ...
  void load_furniture_polygons() {
    ...
    if (!add_static_obstacles_) {
      RCLCPP_INFO(this->get_logger(),
                  "AugmentedGridMap.-> add_static_obstacles is false; skipping "
                  "static furniture obstacles");
      return;
    }
    const std::string yaml_path =
        augment_gridmap_online::resolve_furniture_yaml_path(
            furniture_pkg_, furniture_path_, furniture_yaml_file_,
            this->get_logger());
    if (yaml_path.empty()) {
      RCLCPP_WARN(this->get_logger(),
                  "AugmentedGridMap.-> add_static_obstacles is true but no "
                  "YAML path resolved; skipping");
      return;
    }
    furniture_polygons_ =
        augment_gridmap_online::load_furniture_polygons_from_yaml(
            yaml_path, this->get_logger(), debug_);
  }

  void apply_furniture_obstacles() {
    if (furniture_polygons_.empty()) {
      return;
    }
    if (enhanced_map_.data.empty()) {
      RCLCPP_WARN(this->get_logger(),
                  "AugmentedGridMap.-> No map yet; furniture obstacles will be "
                  "applied on next map");
      return;
    }
    for (const auto &poly : furniture_polygons_) {
      const int filled =
          augment_gridmap_online::stamp_polygon_to_grid(enhanced_map_, poly);
      if (debug_) {
        RCLCPP_INFO(this->get_logger(),
                    "AugmentedGridMap.-> Stamped %d cell(s) for furniture '%s'",
                    filled, poly.name.c_str());
      }
    }
    pub_furniture_marker_->publish(
        augment_gridmap_online::build_furniture_markers(
            furniture_polygons_, enhanced_map_.header.frame_id, this->now()));
  }
};
