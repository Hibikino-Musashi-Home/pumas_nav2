...
public:
  PotentialFieldsNode()
      : Node("potential_fields_node"), tf_buffer_(this->get_clock()),
    ...
    param_callback_handle_ = this->add_on_set_parameters_callback(
        std::bind(&PotentialFieldsNode::on_parameter_change, this,
                  std::placeholders::_1));
...
private:
  ...
  on_parameter_change(const std::vector<rclcpp::Parameter> &params) {
    ...
    for (const auto &param : params) {
      ...
      if (name == "use_namespace")
      ...
      else if (name == "laser_max_x")
        laser_max_x_ = param.as_double();
      else if (name == "laser_min_y")
        laser_min_y_ = param.as_double();
      else if (name == "laser_max_y")
        laser_max_y_ = param.as_double();
      else if (name == "laser_min_z")
        laser_min_z_ = param.as_double();
      else if (name == "laser_max_z")
        laser_max_z_ = param.as_double();

      else if (name == "cloud_min_x")
        cloud_min_x_ = param.as_double();
      else if (name == "cloud_max_x")
        cloud_max_x_ = param.as_double();
      else if (name == "cloud_min_y")
        cloud_min_y_ = param.as_double();
      else if (name == "cloud_max_y")
        cloud_max_y_ = param.as_double();
      else if (name == "cloud_min_z")
        cloud_min_z_ = param.as_double();
      else if (name == "cloud_max_z")
        cloud_max_z_ = param.as_double();

      else if (name == "laser_pot_fields_d0")
        laser_pot_fields_d0_ = param.as_double();
      else if (name == "cloud_pot_fields_d0")
        cloud_pot_fields_d0_ = param.as_double();
      else if (name == "laser_pot_fields_k_rej")
        laser_pot_fields_k_rej_ = param.as_double();
      else if (name == "cloud_pot_fields_k_rej")
        cloud_pot_fields_k_rej_ = param.as_double();
      else if (name == "no_sensor_data_timeout")
        no_sensor_data_timeout_ = param.as_double();

      else if (name == "cloud_points_threshold")
        cloud_threshold_ = param.as_int();
      else if (name == "cloud_downsampling")
        cloud_downsampling_ = param.as_int();
      else if (name == "lidar_points_threshold")
        lidar_threshold_ = param.as_int();
      else if (name == "lidar_downsampling")
        lidar_downsampling_ = param.as_int();

      else if (name == "point_cloud_topic")
        point_cloud_topic_ = param.as_string();
      else if (name == "laser_scan_topic")
        laser_scan_topic_ = param.as_string();
      else if (name == "base_link_name")
        ...
        RCLCPP_WARN(
            this->get_logger(),
            "PotentialFields.-> Attempted to update unsupported parameter: %s",
            name.c_str());
        break;
      }
    }
  ...
  void wait_for_transforms(const std::string &target_frame,
    ...
    if (!transform_ok) {
      RCLCPP_WARN(this->get_logger(),
                  "PotentialFields.-> Timeout while waiting for transform from "
                  "'%s' to '%s'.",
                  source_frame.c_str(), target_frame.c_str());
    } else {
      RCLCPP_INFO(
  ...
  callback_point_cloud(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
    try {
      ...
      RCLCPP_ERROR(
          this->get_logger(),
          "PotentialFields.-> Error processing callback_point_cloud: %s",
          e.what());
    }
  }
  ...
  void callback_lidar(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
    try {
      ...
      RCLCPP_ERROR(this->get_logger(),
                   "PotentialFields.-> Error processing callback_lidar: %s",
                   e.what());
    }
  }
  ...
  void callback_enable(const std_msgs::msg::Bool::SharedPtr msg) {
    try {
      ...
      if (enable_) {
        ...
        RCLCPP_INFO(this->get_logger(),
                    "PotentialFields.->Stopping obstacle detection...");
      ...
      RCLCPP_ERROR(this->get_logger(),
                   "PotentialFields.-> Error processing callback_enable: %s",
                   e.what());
    }
  }
  ...
  void callback_cmd_vel(const geometry_msgs::msg::Twist::SharedPtr msg) {
    try {
      ...
      RCLCPP_ERROR(this->get_logger(),
                   "PotentialFields.-> Error processing callback_cmd_vel: %s",
                   e.what());
    }
  }
  ...
  void callback_goal_path(const nav_msgs::msg::Path::SharedPtr msg) {
    try {
      ...
      RCLCPP_ERROR(this->get_logger(),
                   "PotentialFields.-> Error processing callback_goal_path: %s",
                   e.what());
    }
  }
  ...
  bool check_collision_risk_with_lidar(
    ...
    for (size_t i = 0; i < msg->ranges.size(); i += lidar_downsampling_) {
      ...
      if (v.x() > laser_min_x_ && v.x() < optimal_x && v.y() > laser_min_y_ &&
          v.y() < laser_max_y_ && v.z() > laser_min_z_ &&
          v.z() < laser_max_z_) {
        obstacle_count++;
      }
      ...
      if (distance < laser_pot_fields_d0_) {
        double force_mag =
            laser_pot_fields_k_rej_ *
            std::sqrt(1.0 / distance - 1.0 / laser_pot_fields_d0_);
    ...
    if (debug_) {
      if (obstacle_count > lidar_threshold_ && rejection_force_y != 0.0) {
        RCLCPP_INFO(this->get_logger(),
                    "PotentialFields.-> Laser->Rejection force: x=%.3f y=%.3f",
  ...
  bool check_collision_risk_with_cloud(
    ...
    for (size_t i = 0; i < msg->width * msg->height;
      ...
      if ( // v.norm() < cloud_pot_fields_d0_ &&
        ...
        mask.data[i] = 255;
        float force_mag =
            cloud_pot_fields_k_rej_ *
            std::sqrt(1.0 / v.norm() - 1.0 / cloud_pot_fields_d0_);
    ...
    rejection_force_x =
        (force_count > 0) ? rejection_force_x / force_count : 0.0;
    rejection_force_y =
        (force_count > 0) ? rejection_force_y / force_count : 0.0;
  ...
  float get_search_distance(float max_x) {
    ...
    float dist_to_goal =
        sqrt(pow(global_goal_x_ - robot_x, 2) +
             pow(global_goal_y_ - robot_y, 2)); // TODO x->y fixbug by r.k
    if (dist_to_goal < max_x)
      return dist_to_goal;
    return max_x;
  }

  void get_robot_position_wrt_map(float &robot_x, float &robot_y,
                                  float &robot_t) {
    try {
      ...
      // RCLCPP_WARN(this->get_logger(), "PotentialFields.-> TF Exception: %s",
      // ex.what());
  ...
  get_force_arrow_markers(const geometry_msgs::msg::Vector3 &f1,
  ...
  void potential_fields_processing() {
    try {
      ...
      if (use_pot_fields_) {
        ...
        msg_rejection_force.x =
            rejection_force_lidar_.x + rejection_force_cloud_.x;
        msg_rejection_force.y =
            rejection_force_lidar_.y + rejection_force_cloud_.y;

        // if (rejection_force_lidar_.y > 0 && rejection_force_cloud_.y > 0) {
        // //fix logic by r.k
        if (rejection_force_lidar_.y != 0 && rejection_force_cloud_.y != 0) {
          msg_rejection_force.x =
              (rejection_force_lidar_.x + rejection_force_cloud_.x) / 2.0;
          msg_rejection_force.y =
              (rejection_force_lidar_.y + rejection_force_cloud_.y) / 2.0;
        }
      ...
      RCLCPP_ERROR(this->get_logger(),
                   "PotentialFields.-> Error in PotentialFields processing: %s",
                   e.what());
    }
  }
};
